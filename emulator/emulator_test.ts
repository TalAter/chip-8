...
describe("decodeAndExecute", () => {
    ...
    describe("8XY1", () => {
        it("sets VX to the OR of VX and VY. Does not affect VY", () => {
            const testCases = [
                {
                    input: { vx: 0x00, vy: 0x00 },
                    expected: { vx: 0x00, vy: 0x00 },
                },
                {
                    input: { vx: 0xFF, vy: 0x00 },
                    expected: { vx: 0xFF, vy: 0x00 },
                },
                {
                    input: { vx: 0x00, vy: 0xFF },
                    expected: { vx: 0xFF, vy: 0xFF },
                },
                {
                    input: { vx: 0xF0, vy: 0x0F },
                    expected: { vx: 0xFF, vy: 0x0F },
                },
                {
                    input: { vx: 0xA5, vy: 0x5A },
                    expected: { vx: 0xFF, vy: 0x5A },
                },
            ];
            ...
            testCases.forEach(({ input, expected }) => {
                ...
                emulator.decodeAndExecute(0x8011);
    ...
    describe("9XY0", () => {
        it("skips one instruction if the value in VX is not equal to VY", () => {
            const testCases = [
                { vx: 0x42, vy: 0x24, shouldSkip: true }, // Different values
                { vx: 0x00, vy: 0x01, shouldSkip: true }, // Zero and non-zero
                { vx: 0x42, vy: 0x42, shouldSkip: false }, // Equal values
                { vx: 0x00, vy: 0x00, shouldSkip: false }, // Zero case
                { vx: 0xFF, vy: 0xFF, shouldSkip: false }, // Max value case
            ];
            ...
            testCases.forEach(({ vx, vy, shouldSkip }) => {
                ...
                setRegister(2, vx);
                setRegister(3, vy);
                emulator.decodeAndExecute(0x9230);
                expect(getPC()).toBe(shouldSkip ? PC + 2 : PC);
            });
        });
    });
